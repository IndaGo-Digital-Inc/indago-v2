...
export function useGlitchAnimation(
    ...
    function applyGlitchStyle(el: HTMLElement) {
        const color = pickWeightedColor(config, animationState.value);
        const transitionDuration =
            (
                config.minTransition +
                Math.random() * (config.maxTransition - config.minTransition)
            ).toFixed(2) + "s";
        const rotX =
            (
                config.minRotate +
                Math.random() * (config.maxRotate - config.minRotate)
            ).toFixed(2) + "deg";
        const rotY =
            (
                config.minRotate +
        ...
        const skewX =
            (
                config.minSkew +
                Math.random() * (config.maxSkew - config.minSkew)
            ).toFixed(2) + "deg";
        const skewY =
            (
                config.minSkew +
    ...
    function applyGlitchesToCandidates(
